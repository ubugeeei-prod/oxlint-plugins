...
ESLint plugins used by [flyle-nexus] that Oxlint does not support natively, collected here as port targets. `eslint-plugin-svelte` is intentionally excluded — it is handled by [rsvelte](https://github.com/baseballyama/rsvelte). Oxlint-supported plugins (`typescript-eslint`, `eslint-plugin-import`, `eslint-plugin-n`, `eslint-plugin-unicorn`) are used directly via Oxlint and are not listed here.
