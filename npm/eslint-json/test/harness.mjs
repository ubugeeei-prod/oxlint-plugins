// Replay harness for upstream @eslint/json RuleTester cases.
//
...
// (Oxlint's column space); RuleTester asserts 1-indexed columns, so the harness
// adds 1 to every column. The JSON dialect (`language`) does not change the
// permissive Rust scanner's behavior, so it is preserved only for the label.
...
export function runRule(ruleName, testCase) {
  ...
  const context = {
    ...
    sourceCode,
    filename: testCase.filename ?? 'file.json',
    report(descriptor) {
...
// (ignoring `type`/`suggestions`, which the JSON port does not model).
