// Captures the stable @stylistic/jsx-first-prop-new-line v5.10.0 RuleTester
// suite from the exact pinned upstream commit, including ESLint 10 parser
// variants, then enriches every case with published-rule diagnostics and fixes.
...
const RULE = 'jsx-first-prop-new-line';
...
const CAPTURE_KEY = '__stylisticJsxFirstPropNewLineCapture__';
...
const captureDir = mkdtempSync(join(tmpdir(), 'stylistic-jsx-first-prop-new-line-capture-'));
...
function enrichWithPublishedRule(cases: { valid: NormalizedCase[]; invalid: NormalizedCase[] }): {
  ...
  const runnerDir = mkdtempSync(join(tmpdir(), 'stylistic-jsx-first-prop-new-line-upstream-'));
  try {
    writeFileSync(
