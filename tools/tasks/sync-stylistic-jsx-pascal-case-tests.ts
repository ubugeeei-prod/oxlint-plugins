// Captures stable @stylistic/jsx-pascal-case v5.10.0 from the exact
// upstream commit, including ESLint 10's parser expansion, then enriches every
// invalid case with published-rule diagnostics and exact non-fixable output.
...
const CAPTURE_KEY = '__stylisticJsxPascalCaseCapture__';
...
const captureDir = mkdtempSync(join(tmpdir(), 'stylistic-jsx-pascal-case-capture-'));
try {
  ...
  const fixture = {
    __generated: {
      ...
      inventory: {
        ...
        diagnostics,
        fixableInvalid: 0,
        unfixableInvalid: enriched.invalid.length,
  ...
  console.log(
    `Synced @stylistic/${RULE} v${VERSION} (${PINNED_COMMIT}): ` +
      ...
      `${diagnostics} diagnostics (all non-fixable).`,
  );
} finally {
...
function normalizeCase(raw: RawCase, invalid: boolean, label: string): NormalizedCase {
  ...
  const allowed = new Set(
    invalid
      ? ['code', 'options', 'parserOptions', 'errors', 'parser']
...
function enrichWithPublishedRule(cases: { valid: NormalizedCase[]; invalid: NormalizedCase[] }): {
  ...
  const runnerDir = mkdtempSync(join(tmpdir(), 'stylistic-jsx-pascal-case-upstream-'));
  try {
    writeFileSync(
...
function diagnostic(message) {
  ...
  return {
    ...
    data: message.messageId === 'usePascalCase' || message.messageId === 'usePascalOrSnakeCase'
      ? { name: message.message.match(/^Imported JSX component (.+) must be/u)?.[1] }
      : {},
    range: [start, end],
    loc: {
      line: message.line,
...
const invalid = captured.invalid.map((testCase, index) => {
  ...
  for (const [messageIndex, expected] of testCase.upstreamErrors.entries()) {
    ...
    if (JSON.stringify(expected.data ?? {}) !== JSON.stringify(diagnostic(actual).data)) {
      throw new Error(
        'Published rule data differs for invalid case ' + index + ', message ' + messageIndex,
      );
    }
    ...
    if (actual.fix !== undefined) {
      throw new Error('Published rule unexpectedly fixed invalid case ' + index);
    }
  }
  const fixed = new Linter().verifyAndFix(testCase.code, configFor(testCase), {
    filename: filenameFor(testCase),
  });
  if (fixed.fixed || fixed.output !== testCase.code) {
    throw new Error('Published rule unexpectedly changed invalid case ' + index);
  }
  return {
    code: testCase.code,
    options: testCase.options,
...
function addParser(testCase, parser, features) {
  ...
  return { ...rest, code: value.code + extraComment, parser };
}
function expand(tests) {
  return tests.flat(Infinity).filter(Boolean).flatMap(testCase => {
    const value = typeof testCase === 'string' ? { code: testCase } : testCase;
