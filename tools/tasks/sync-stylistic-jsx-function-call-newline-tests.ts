// Captures every authored stable @stylistic/jsx-function-call-newline v5.10.0
// RuleTester case from the exact pinned upstream commit. The upstream parser
// matrix repeats authored JSX cases across compatible parsers; this fixture
// stores each semantic case once and native tests replay both JSX and TSX.
...
const RULE = 'jsx-function-call-newline';
...
const CAPTURE_KEY = '__stylisticJsxFunctionCallNewlineCapture__';
...
const temp = mkdtempSync(join(tmpdir(), 'stylistic-jsx-function-call-newline-sync-'));
...
function normalizeCase(raw: RawCase, invalid: boolean, index: number): Record<string, unknown> {
  ...
  if (invalid) {
    normalized.errors = (normalized.errors as Array<Record<string, unknown>>).map((error) => {
      if (error.messageId !== 'missingLineBreak') {
