...
//
// Re-run with `pnpm run port:tests:stylistic:indent-binary-ops`.
...
const CAPTURE_KEY = '__stylisticIndentBinaryOpsSyncCapture__';
...
const tempDir = mkdtempSync(join(tmpdir(), 'stylistic-indent-binary-ops-sync-'));
...
try {
  ...
  if (runs[0].recursive !== 10) {
    throw new Error(`Expected recursive fix limit 10, received ${runs[0].recursive}.`);
  }
