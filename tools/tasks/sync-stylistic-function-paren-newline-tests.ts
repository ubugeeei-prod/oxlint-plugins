...
// vendored v5.10.0 submodule, then asks the published v5.10.0 rule for the
// exact diagnostic locations, fixes, and iterative fixed output. The resulting
// committed JSON is replayed by both the Rust and JavaScript test suites.
//
// Re-run with `pnpm run port:tests:stylistic:function-paren-newline`.
...
const RULE = 'function-paren-newline';
...
const CAPTURE_KEY = '__stylisticFunctionParenNewlineCapture__';
...
const captureDir = mkdtempSync(join(tmpdir(), 'stylistic-function-paren-capture-'));
...
const fixture = {
  __generated: {
    ...
    tool: 'tools/tasks/sync-stylistic-function-paren-newline-tests.ts',
  },
...
function enrichWithPublishedRule(captured: EnrichedFixture): EnrichedFixture {
  const runnerDir = mkdtempSync(join(tmpdir(), 'stylistic-function-paren-upstream-'));
  try {
    writeFileSync(
...
function registerCaptureHooks(): void {
  ...
  registerHooks({
    resolve(specifier, context, nextResolve) {
      ...
      if (
        specifier === './function-paren-newline' ||
        specifier === './types' ||
