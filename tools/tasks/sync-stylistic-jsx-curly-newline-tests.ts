// Captures the stable @stylistic/jsx-curly-newline v5.10.0 RuleTester suite
// from the exact pinned upstream commit, including its ESLint 10 parser variants,
// then enriches every case with published-rule diagnostics and fixes.
...
const CAPTURE_KEY = '__stylisticJsxCurlyNewlineCapture__';
...
const captureDir = mkdtempSync(join(tmpdir(), 'stylistic-jsx-curly-newline-capture-'));
...
function enrichWithPublishedRule(cases: { valid: NormalizedCase[]; invalid: NormalizedCase[] }): {
  ...
  const runnerDir = mkdtempSync(join(tmpdir(), 'stylistic-jsx-curly-newline-upstream-'));
  try {
    writeFileSync(
...
function parserConfiguration(testCase) {
...
function configFor(testCase) {
  const parser = parserConfiguration(testCase);
  return [{
    files: ['**/*.jsx', '**/*.tsx'],
    languageOptions: {
      ecmaVersion: 'latest',
...
function registerCaptureHooks(): void {
  ...
  const parsersStub = `
function addParser(testCase, parser) {
...
function expand(tests) {
  return tests.flat().filter(Boolean).flatMap(testCase =>
    ['espree', 'typescript'].map(parser => addParser(testCase, parser))
  );
}
