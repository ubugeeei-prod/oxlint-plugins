// Captures both stable @stylistic/eslint-plugin brace-style RuleTester suites
// from the pinned vendored submodule as a committed JSON fixture.
//
// Re-run with `pnpm run port:tests:stylistic:brace-style`.
...
const CAPTURE_KEY = '__stylisticBraceStyleSyncCapture__';
...
const tempDir = mkdtempSync(join(tmpdir(), 'stylistic-brace-style-sync-'));
...
try {
  ...
  const fixture = {
    __generated: {
      ...
      license: 'MIT',
      tool: 'tools/tasks/sync-stylistic-brace-style-tests.ts',
    },
    valid,
    invalid,
  };
