// Captures every authored stable @stylistic/jsx-max-props-per-line v5.10.0
// RuleTester case from the exact pinned upstream commit. The upstream parser
// matrix repeats each authored JSX case across compatible parsers; this fixture
// stores each semantic case once and the native replay runs it through Oxc's
// JSX/TSX parser.
...
const RULE = 'jsx-max-props-per-line';
...
const CAPTURE_KEY = '__stylisticJsxMaxPropsPerLineCapture__';
...
const temp = mkdtempSync(join(tmpdir(), 'stylistic-jsx-max-props-per-line-sync-'));
...
function normalizeCase(raw: RawCase, invalid: boolean, index: number): Record<string, unknown> {
  ...
  if (invalid) {
    normalized.errors = (normalized.errors as Array<Record<string, unknown>>).map((error) => {
      const data = error.data as { prop?: unknown } | undefined;
      if (error.messageId !== 'newLine' || typeof data?.prop !== 'string') {
        throw new Error(`Invalid case ${index} has an unsupported error contract.`);
      }
      return {
        ...error,
        message: `Prop \`${data.prop}\` must be placed on a new line`,
      };
    });
  }
  return normalized;
