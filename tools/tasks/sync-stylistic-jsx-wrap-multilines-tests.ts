// Captures every authored stable @stylistic/jsx-wrap-multilines v5.10.0
// RuleTester case from the exact pinned upstream commit. The upstream parser
// matrix repeats compatible JSX cases; this fixture stores every authored
// semantic case once and replays it through Oxc's JSX and TSX parser.
...
const RULE = 'jsx-wrap-multilines';
...
const CAPTURE_KEY = '__stylisticJsxWrapMultilinesCapture__';
...
const temp = mkdtempSync(join(tmpdir(), 'stylistic-jsx-wrap-multilines-sync-'));
...
function normalizeCase(raw: RawCase, invalid: boolean, index: number): Record<string, unknown> {
  ...
  if (invalid) {
    normalized.errors = (normalized.errors as Array<Record<string, unknown>>).map((error) => {
      ...
      if (messageId !== 'missingParens' && messageId !== 'parensOnNewLines') {
        throw new Error(`Invalid case ${index} has unsupported message ${String(messageId)}.`);
      }
      return {
        ...error,
        message:
